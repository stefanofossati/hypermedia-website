...
<template>
  <div> <!-- page with team's profile pictures and names -->
    <div class="flex flex-col items-center text-center">
      ...
      <div class="flex flex-col items-center md:grid md:place-items-center md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5 w-full px-5">
        <div v-for="person in team">
          <NuxtLink :to="`/team/${person.name}_${person.surname}`"
            ...
            <img :src="person.img" :alt="`${person.name} ${person.surname}'s profile pic`" class="rounded-b-xl h-64 md:h-72 lg:h-96 w-full"/>
          </NuxtLink>
        </div>
      </div>
    </div>
