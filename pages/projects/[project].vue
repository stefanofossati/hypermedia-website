<template>
  <div class="divide-y divide-slate-500">
    ...
    <div class="sm:grid sm:grid-cols-4 sm:gap-3 py-3 px-5 block">
      ...
      <div
          class="sm:col-span-1 sm:sticky sm:inset-y-0 sm:right-0 sm:flex sm:flex-col border-l-2 border-slate-700 dark:border-gray-500 hidden">
        <ProjectsProjectInfo :info_supervisor="pj.supervisor" :info_budget="pj.budget" :info_start_date="pj.start_date"
                             :info_end_date="pj.end_date"/>
      </div>
    </div>
    ...
    <div class="py-4">
      <div class="grid grid-cols-12 grid-rows-4 gap 3 justify-items-center place-items-center">
        ...
        <div class="col-span-4 row-span-4 self-center">
          <img id="big_image" :src="pj.gallery_images[0].url" alt="" class=" h-96 w-full object-cover">
        </div>
        <div class="col-span-7 row-span-1 text-black dark:text-white text-3xl">
          Project Gallery
        </div>
        ...
        <div  class="col-span-2 row-span-2 bg-gray-100 " >
          <img id="image01" :src="pj.gallery_images[1].url" alt="" class=" h-48 w-full object-cover " >
        </div>
        <div  class="col-span-2 row-span-2 " >
          <img id="image02" :src="pj.gallery_images[2].url" alt=""  class=" h-48 w-full object-cover ">
        </div>
        <div  class="col-span-2 row-span-2 bg-gray-100 ">
          <img id="image03" :src="pj.gallery_images[3].url" alt="" class=" h-48 w-full object-cover ">
        </div>
        ...
        <div class="col-span-1 row-span-1"></div>
        <div id="description_big_image" class="col-span-7 row-span-1 justify-self-start text-black dark:text-white">
          {{ pj.gallery_images[i].description }}
        </div>
...
function go_left(length: number, images_src: ImageGallery[]) {
  ...
  i = (i+1) % (length);
  big_image.src = images_src[i].url;
  description_big_image.innerHTML = images_src[i].description;

  let j = (i+1) % (length);
  image_01.src = images_src[j].url;

  let k = (i+2) % (length);
  image_02.src = images_src[k].url;

  let l = (i+3) % (length);
  image_03.src = images_src[l].url;
